...
import Media from "../../app/projets/_components/media";
...
export default function Tile({
  ...
  return status === "Draft" ? (
      ...
      <div className="overflow-hidden w-full h-full rounded-3xl opacity-30 grayscale transition-all duration-700">
        ...
        <div className="flex absolute top-0 left-0 z-10 flex-col justify-between items-end p-6 w-full h-full backdrop-brightness-90 transition-all duration-700 lg:p-8 group-hover:backdrop-brightness-75">
          <div className="flex flex-col w-full">
            ...
            <h2 className="flex mb-4 text-3xl font-semibold text-white">
              {title}
            </h2>
          </div>
        </div>
    ...
    >
      <div className="overflow-hidden w-full h-full rounded-3xl transition-all duration-700 scale-100 lg:group-hover:scale-103">
        <Media type={img.type} url={img.url} name={img.name} cover />
        <div className="flex absolute top-0 left-0 z-10 flex-col justify-between items-end p-6 w-full h-full backdrop-brightness-90 transition-all duration-700 lg:p-8 lg:group-hover:backdrop-brightness-75">
          <div className="flex flex-col w-full">
            <div className="flex flex-wrap gap-y-0 gap-x-4 mb-4">
              {tags.map((tag, index) => (
                <p
