  ...
  function filteredProjects(skill: SkillsList) {
    ...
    if (skill === "Motion Design") {
      return (
        ...
        projects.filter((p) => p.tags.find((tag) => tag === "Motion Design"))[0]
          ?.img
      );
    }
            ...
            filteredProjects(skill) !== undefined && (
              ...
              >
                <Link href={`/projets?filter=${skill}`}>
                  <h2 className=" whitespace-nowrap text-white mix-blend-difference relative text-[12vw] 2xl:text-[9vw] anton uppercase translate-0 group-hover:-translate-x-10 max-lg:group-hover:translate-x-0 will-change-auto transition-all duration-300 font-bold leading-[0.8] group-hover:z-20">
                    {skill}
                  </h2>
                </Link>
                <motion.div
                  style={{ left: -position.x / 6, top: -position.y / 3 }}
                  className="absolute rounded-3xl overflow-hidden items-center justify-center opacity-0 transition-[scale] w-[20vw] will-change-auto max-lg:hidden group-hover:opacity-100 flex aspect-4/5 scale-0 group-hover:scale-100 group-hover:z-10"
                >
                  {skill && (
                    <Media
